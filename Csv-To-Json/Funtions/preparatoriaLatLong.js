...
const jsonFilePath = path.join(__dirname, "../Converter/PreparatoriaValidados01.json");
const jsonCompletFilePath = path.join(
  __dirname,
  "../ConverterComplet/PreparatoriaValidados00.json"
);
// Convirtiendolo para poder usarlo
const jsonFile = JSON.parse(fs.readFileSync(jsonFilePath, "utf-8"));

const getLatLong = async () => {
  for (let i = 515; i < jsonFile.length ; i++) {
